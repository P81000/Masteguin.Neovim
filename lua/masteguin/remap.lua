...
-- Mapeamento global: mostra aviso se nao for .c, .cpp ou .h
...
-- Atalhos especificos para arquivos C/C++/header
vim.api.nvim_create_autocmd("FileType", {
  group = "CommentCFiles",
  pattern = { "c", "cpp", "h" },
  callback = function()
    -- Comentar: adiciona // no inicio das linhas selecionadas
    vim.keymap.set("v", "<leader>c", [[:s/^/\/\/ /<CR>]], { buffer = true, noremap = true })

    -- Descomentar: remove // e espaco se estiverem no inicio da linha
